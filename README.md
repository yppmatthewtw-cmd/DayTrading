...
### 深紅動能見底訊號 (`scripts/bottom_signal.py`)

深紅柱（`h<0` 且仍在變大）代表跌勢**仍在加速**，本身永遠不會是底。
要捉的是「深紅耗盡的那一刻」，分五級：

| 級別 | 判定 | 距真底 | 其後20根報酬 |
|---|---|---|---|
| B1 深紅減速 | 仍深紅，但 `\|Δh\|` 由本段峰值收窄 ≥40% | 早 5.5 根 | +2.4% |
| B2 首根淺紅 | 深紅 → 淺紅 | 早 3.5 根 | **+4.3%** |
| B3 淺紅確認 | 連續 k 根淺紅 | 早 2.5 根 | +4.4% |
| B4 柱體翻正 | `h>0`（即一般說的金叉） | **遲 6 根** | +1.1% |
| B5 破結構 | 升穿前 20 根高點 | 遲 10 根 | +8.7% |

兩個實測結論（合成資料，基準 +0.83%）：

- **`min_dark_run` (N) 控制出手頻率，不控制準確度。** N 由 2 增到 13，
  觸發次數 17→3，但誤差中位數一直是 3 根左右。
- **控制提前量的是 MACD 週期。** 5/13/5 提前 7 根、誤差 7 根；
  24/52/18 遲 1 根、誤差 1 根。**早與準是同一個數字。**

```bash
python3 scripts/bottom_signal.py --csv bars.csv --calibrate   # 量出自己標的的參數
python3 scripts/bottom_signal.py --csv bars.csv --sweep
```

輸出說明書：`output/2026-09-17_深紅動能見底訊號_參數設定.xlsx`
